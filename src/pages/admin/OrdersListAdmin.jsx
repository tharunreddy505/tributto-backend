...
import { faBox, faCheck, faClock, faTimes, faEye, faXmark, faEnvelope, faMapMarkerAlt, faCalendarAlt, faShoppingBag, faEuroSign } from '@fortawesome/free-solid-svg-icons';
...
const OrdersListAdmin = () => {
    ...
    const stats = [
        ...
        {
            title: 'Total Revenue',
            value: `€ ${orders.filter(o => o.status === 'paid' || o.status === 'shipped').reduce((sum, o) => sum + parseFloat(o.total_amount || 0), 0).toFixed(2)}`,
            icon: faEuroSign,
        ...
        {
            title: 'Completed',
            value: orders.filter(o => o.status === 'paid' || o.status === 'shipped').length,
            icon: faCheck,
                                            ...
                                            <div key={idx} className="flex items-center justify-between gap-4">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        {/* Linked Memorial Badge */}
                                                        {item.metadata && (item.metadata.memorial_name || item.metadata.memorial_id) && (
                                                            <div className="mt-2 text-[10px] bg-amber-50 text-amber-600 border border-amber-100 rounded-md px-2 py-1 flex items-center gap-1.5 w-fit">
                                                                <span>📖 {item.metadata.memorial_name || 'Memorial'}</span>
                                                                {item.metadata.memorial_id && <span className="opacity-50">#{item.metadata.memorial_id}</span>}
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
