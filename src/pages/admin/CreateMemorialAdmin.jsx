...
import { faSave, faArrowLeft, faImage, faUpload, faTrash, faVideo, faPlus, faLink, faTimes } from '@fortawesome/free-solid-svg-icons';
...
const CreateMemorialAdmin = () => {
    ...
    const { addTribute, addMedia, showToast } = useTributeContext();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const createdTribute = await addTribute(newTributeData);

            if (createdTribute && createdTribute.id) {
                if (formData.images.length > 0) {
                    for (const img of formData.images) {
                        await addMedia(createdTribute.id, 'image', img);
                    }
                }
                if (formData.videos.length > 0) {
                    for (const vid of formData.videos) {
                        await addMedia(createdTribute.id, 'video', vid);
                    }
                }
            ...
            showToast("An error occurred while creating memorial.", "error");
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20">
                        ...
                        <div className="mb-6">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                                {formData.images.map((img, index) => (
                                    <div key={index} className="relative aspect-square rounded-lg overflow-hidden group bg-gray-100 border border-gray-200">
                                        <img src={img} alt={`Gallery ${index}`} className="w-full h-full object-cover" />
                                        <button
                                            type="button" onClick={() => removeGalleryItem('images', index)}
                                            className="absolute top-1 right-1 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity shadow-sm"
                                        >
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                                {formData.videos.map((vid, index) => (
                                    <div key={index} className="relative aspect-video rounded-lg overflow-hidden group bg-black border border-gray-200">
                                        ...
                                        <button
                                            type="button" onClick={() => removeGalleryItem('videos', index)}
                ...
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
                        ...
                        >
                            {loading ? 'Saving...' : <><FontAwesomeIcon icon={faSave} /> Publish</>}
                        </button>
                    </div>
